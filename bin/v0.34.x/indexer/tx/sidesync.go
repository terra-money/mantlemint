...
import (
	...
	block2 "github.com/terra-money/mantlemint-provider-v0.34.x/indexer/block"
	"io/ioutil"
	"net/http"
	"os"
	"strconv"
...
func SidesyncTx() {
	...
	for {
		...
		for _, target := range targets {
			r, err := http.Post(fmt.Sprintf("%v%s", target, block2.EndpointPOSTBlock), "application/json", bytes.NewBuffer(input))
			if err != nil {
				panic(err)
			}
